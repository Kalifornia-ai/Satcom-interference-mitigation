#!/usr/bin/env python3
"""
evaluate_beacon_net_multi.py – compare ≥1 checkpoints (CNN, Hybrid, LSTM, …).

* Uses the test-idx saved in cw_power_train_meta.json.
* Accepts state-dict (.pt) or TorchScript (.pth/.pt).
* Converts every model’s output to dBm so the metrics are comparable.
"""

from pathlib import Path
import argparse, csv, json, math, re
from collections import defaultdict

import numpy as np
import torch, torch.nn.functional as F
import matplotlib.pyplot as plt
from tqdm import tqdm
from scipy.fft import fft

from model  import BeaconPowerCNN
from resmodel import HybridBeaconEstimator
from lstm    import LSTMSeperatorSingle     # name your files accordingly

# ─────────────────── CLI ───────────────────────────────────────────────
ap = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
ap.add_argument("--data",   required=True, type=Path)
ap.add_argument("--ckpts",  nargs="+", required=True, type=Path)
ap.add_argument("--outdir", default="eval_multi", type=Path)
ap.add_argument("--meta",   default="cw_power_train_meta.json", type=Path)
ap.add_argument("--len-override", type=int, default=None,
                help="force input length for **all** models")
args = ap.parse_args(); args.outdir.mkdir(parents=True, exist_ok=True)

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
cm     = plt.get_cmap("tab10")
dBm_factor = 10.0 / math.log(10)
GAIN = 0.375

PAT_CW   = re.compile(r"Sine_50002[-_]?(-?\d+)dBm")
PAT_QPSK = re.compile(r"QPSK_5G[-_]?(-?\d+)dBm")
parse_levels = lambda name: (
    int(PAT_CW.search(name).group(1)),
    int(PAT_QPSK.search(name).group(1)),
    -int(PAT_QPSK.search(name).group(1)) - int(PAT_CW.search(name).group(1))
)

def gain_to_dBm(g: complex) -> float:
    return dBm_factor * math.log((abs(g)**2) / 1e-3)

def fft_3bin_amp(x: np.ndarray) -> float:
    X = fft(x * np.hanning(len(x))) / len(x)
    return np.sqrt(np.sum(np.abs(X[[0, 1, -1]])**2) / GAIN)

def seq_to_gain(t: torch.Tensor) -> torch.complex64:
    re = t[..., 0].mean(dim=1);  im = t[..., 1].mean(dim=1)
    return torch.complex(re, im)

# ──────────────── load models ──────────────────────────────────────────
nets, labels, colors, in_len = [], [], [], []

def pick_class(sd):
    k = sd.keys()
    if "front.0.weight"      in k: return BeaconPowerCNN
    if "res.0.c1.weight"     in k: return HybridBeaconEstimator
    if "lstm.0.weight_ih_l0" in k: return LSTMSeperatorSingle
    raise RuntimeError("unknown state-dict format")

for i, ck in enumerate(args.ckpts):
    try:                                         # state-dict?
        sd  = torch.load(ck, map_location=DEVICE, weights_only=True)
        Net = pick_class(sd)
        net = Net().to(DEVICE); net.load_state_dict(sd)
        exp_len = 1000
    except (RuntimeError, TypeError, FileNotFoundError):
        net = torch.jit.load(ck, map_location=DEVICE)         # scripted
        exp_len = 1000
        plist = list(net.parameters())
        if plist and plist[0].dim() == 3:
            k = plist[0].shape[-1]
            if k >= 50: exp_len = k
    net.eval()
    nets.append(net); labels.append(ck.stem)
    colors.append(cm(i % 10)); in_len.append(exp_len)

if args.len_override:
    in_len = [args.len_override]*len(in_len)

# ─────────────── test-file list via meta JSON ──────────────────────────
meta = json.loads((args.data / args.meta).read_text())
test_idx = np.array(meta["test_idx"], dtype=int)
files = np.array(sorted(args.data.rglob("*.npz")))[test_idx]
print(f"Evaluating {len(files)} bursts …")

# ─────────────── containers ────────────────────────────────────────────
by_sir      = [defaultdict(list) for _ in nets]
by_sir_fft  = defaultdict(list)
by_cw_qp    = [defaultdict(lambda: defaultdict(list)) for _ in nets]
by_cw_qp_fft= defaultdict(lambda: defaultdict(list))
plotted_sir = set()

def db_spectrum(sig):
    N=len(sig); S=np.fft.fftshift(np.fft.fft(sig*np.hanning(N)))/N
    f=np.fft.fftshift(np.fft.fftfreq(N,1/10e6))/1e6
    return f,20*np.log10(np.abs(S)+1e-18)

# ─────────────── main loop ─────────────────────────────────────────────
for f in tqdm(files, unit="file"):
    cw_dBm, qpsk_dBm, sir = parse_levels(f.parent.name)
    with np.load(f, allow_pickle=True) as z:
        x_raw   = z["x"].astype(np.complex64)
        g_ref   = z["meta"].item()["pristine_gain"]
    ref_dBm = gain_to_dBm(g_ref)

    preds = []
    with torch.no_grad():
        for m, net in enumerate(nets):
            N = in_len[m];  burst = x_raw
            if N < len(burst):
                s=(len(burst)-N)//2; burst = burst[s:s+N]
            elif N > len(burst):
                p=(N-len(burst))//2; burst = np.pad(burst,(p,N-len(burst)-p))

            arr = np.stack([burst.real, burst.imag], 0)   # (2,N) channels-first
            x_t = torch.tensor(arr, dtype=torch.float32).unsqueeze(0).to(DEVICE)

            out = net(x_t);  out = out[0] if isinstance(out, tuple) else out
            if isinstance(out, dict): out = out["gain"]

            if out.ndim == 3:                             # (B,T,2) sequence
                g_hat = seq_to_gain(out).cpu().item()
                pred  = gain_to_dBm(g_hat)
            elif out.ndim == 2 and out.shape[-1] == 2:    # (B,2) vector
                g_hat = complex(out[0,0].item(), out[0,1].item())
                pred  = gain_to_dBm(g_hat)
            else:                                         # scalar dB
                pred  = out.cpu().item()
            preds.append(pred)

    # ---------- PSD figure (once per SIR) ------------------------------
    if sir not in plotted_sir:
        f_MHz,P_mix = db_spectrum(x_raw/np.sqrt(np.mean(np.abs(x_raw)**2)))
        y_gt = np.full_like(x_raw, g_ref);  _,P_gt=db_spectrum(y_gt)
        mag_est = math.sqrt(1e-3*10**(preds[0]/10))
        y_hat_nn=np.full_like(x_raw,mag_est*np.exp(1j*np.angle(g_ref)));_,P_hat_nn=db_spectrum(y_hat_nn)
        mag_fft=fft_3bin_amp(x_raw); y_hat_fft=np.full_like(x_raw,mag_fft*np.exp(1j*np.angle(g_ref)));_,P_hat_fft=db_spectrum(y_hat_fft)

        fig,ax=plt.subplots(figsize=(6,3))
        ax.plot(f_MHz,P_mix,lw=.7,label='mixture')
        ax.plot(f_MHz,P_gt,label='ground truth')
        ax.plot(f_MHz,P_hat_nn,'--',label=f'estimate ({labels[0]})')
        ax.plot(f_MHz,P_hat_fft,':',label='FFT 3-bin')
        ax.set(xlabel='Freq (MHz)',ylabel='dB re RMS',xlim=(-5,5),ylim=(-150,0),
               title=f'PSD – SIR {sir} dB'); ax.grid(ls=':')
        ax.legend(frameon=False,ncol=4,fontsize=8)
        fig.tight_layout(); fig.savefig(args.outdir/f'psd_SIR{sir}.png',dpi=220)
        plt.close(fig)
        plotted_sir.add(sir)

    # ---------- accumulate errors -------------------------------------
    for m,p in enumerate(preds):
        by_sir[m][sir].append(p-ref_dBm)
        by_cw_qp[m][cw_dBm][qpsk_dBm].append(p-ref_dBm)
    fft_dBm=10*math.log10((fft_3bin_amp(x_raw)**2)/1e-3)
    err    = fft_dBm-ref_dBm
    by_sir_fft[sir].append(err); by_cw_qp_fft[cw_dBm][qpsk_dBm].append(err)

# ─────────────── summary plots & CSV (unchanged) ───────────────────────
sirs = np.array(sorted(by_sir[0]))
fig,ax=plt.subplots(figsize=(6.5,4))
for col,lbl,m in zip(colors,labels,range(len(nets))):
    ax.plot(sirs,[np.mean(by_sir[m][s]) for s in sirs],'o-',color=col,label=lbl)
ax.plot(sirs,[np.mean(by_sir_fft[s]) for s in sirs],'^--',color='k',label='FFT 3-bin')
ax.set_xlabel('SIR (dB)'); ax.set_ylabel('Mean ΔPower (dB)'); ax.grid(ls=':')
ax.legend(frameon=False); fig.tight_layout()
fig.savefig(args.outdir/'mean_dA_vs_SIR.png',dpi=220); plt.close(fig)

fig,ax=plt.subplots(figsize=(6.5,4))
for col,lbl,m in zip(colors,labels,range(len(nets))):
    ax.plot(sirs,[np.mean(np.abs(by_sir[m][s])) for s in sirs],'o-',color=col,label=lbl)
ax.plot(sirs,[np.mean(np.abs(by_sir_fft[s])) for s in sirs],'^--',color='k',label='FFT 3-bin')
ax.set_xlabel('SIR (dB)'); ax.set_ylabel('Mean |ΔPower| (dB)')
ax.set_title('Absolute error versus SIR'); ax.grid(ls=':')
ax.legend(frameon=False); fig.tight_layout()
fig.savefig(args.outdir/'abs_dA_vs_SIR.png',dpi=220); plt.close(fig)

with (args.outdir/'summary.csv').open('w',newline='') as fh:
    wr=csv.writer(fh)
    hdr=['SIR']+[f'µ_{l}' for l in labels]+['µ_FFT']+\
        [f'σ_{l}' for l in labels]+['σ_FFT']
    wr.writerow(hdr)
    for s in sirs:
        row=[s]+[f"{np.mean(by_sir[m][s]):.3f}" for m in range(len(nets))]+\
            [f"{np.mean(by_sir_fft[s]):.3f}"]+\
            [f"{np.std(by_sir[m][s]):.3f}" for m in range(len(nets))]+\
            [f"{np.std(by_sir_fft[s]):.3f}"]
        wr.writerow(row)
print("✓ Plots & CSV saved →", args.outdir.resolve())
